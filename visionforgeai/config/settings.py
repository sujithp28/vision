...
class Settings(BaseSettings):
    ...
    database_url: str = Field(
        default="sqlite:///./visionforge.db", description="Database connection URL"
    )
    database_echo: bool = Field(default=False, description="Enable SQL query logging")

    # Cache Settings
    redis_url: Optional[str] = Field(
        default=None, description="Redis URL for caching (optional)"
    )
    cache_ttl: int = Field(default=3600, description="Default cache TTL in seconds")

    # Storage Settings
    storage_type: str = Field(
        default="local", description="Storage type (local, s3, gcs, azure)"
    )
    storage_path: str = Field(
        default="./storage", description="Local storage path or bucket name"
    )
    ...
    max_upload_size: int = Field(
        default=100 * 1024 * 1024, description="Maximum file upload size in bytes"
    )
    ...
    output_directory: str = Field(
        default="./output", description="Directory for generated output files"
    )
    temp_directory: str = Field(
        default="./tmp", description="Directory for temporary files"
    )
    ...
    enable_usage_tracking: bool = Field(
        default=True, description="Enable API usage tracking and cost calculation"
    )
    enable_content_moderation: bool = Field(
        default=False, description="Enable content moderation for generated content"
    )
